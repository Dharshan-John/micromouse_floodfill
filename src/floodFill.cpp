...
namespace FloodFill
{
    ...
    void runFloodFill()
    {
        ...
        {
            for (int j = 0; j < 16; j++)
                dist[i][j] = -1; // Initialising every cell to an undefined distance
        }
        ...
        for (int i = 0; i < 2; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                dist[7 + i][7 + j] = 0; // Initialsing the destination cells as zero
                target.push({7 + i, 7 + j});
            }
        }
        ...
        while (!target.empty())
        {
            ...
            for (int it = 0; it < 4; it++)
            {
                ...
                if (neighborX < 0 || neighborX > 15 ||
                    neighborY < 0 || neighborY > 15)
                    continue;
                if (Maze::hasWall(activeX, activeY, currentDir))
                    continue;
    ...
    robot::Direction suggestBestDirection(int currentX, int currentY, robot::Direction currentHeading)
    {
        ...
        {
            robot::Direction currentDir = static_cast<robot::Direction>(it);
            if (Maze::hasWall(currentX, currentY, currentDir))
                continue;
